...
Once the venue control server is running (see `server/`), the agent takes its lock
decisions from there instead of the local pipe:

```powershell
# On the server, enrol the unit and note the token — it is shown once.
curl -X POST 'http://<server>:8000/agents/enroll?unit_id=<id>'

# On the cafe PC:
PlaySlot.Agent.exe --unit=<id> `
                   --server-url=ws://<server>:8000 `
                   --device-token=<token> `
                   --no-control-pipe
```

`--no-control-pipe` matters in production. The pipe has no authentication, so a customer
could unlock their own machine by running the same exe; it exists only for local testing.

Every message is HMAC-SHA256 signed in both directions. `Ipc/MessageSigner.cs` matches the
server's canonical form byte for byte, and `Ipc/signing-vectors.json` — generated by the
server — is the contract between them. Three details in that file are load-bearing and
each one silently breaks every signature if changed:

| Detail | Why |
|---|---|
| `UnsafeRelaxedJsonEscaping` | System.Text.Json escapes `<`, `>` and `&` by default; Python does not |
| `StringComparer.Ordinal` key sort | Culture-aware sorting differs by machine locale |
| No floating point in bodies | Python writes `1.0` where .NET writes `1` |

### The fail-safe (Zone 5)

The server pushes the session's end time with every state message, and the agent caches it
to disk. When the link drops, the agent waits `--failsafe-delay` seconds (default 60) and
then asks one question: **is there paid time remaining?**

| Answer | Action |
|---|---|
| Yes | Stay unlocked, keep counting down locally |
| No, or expired | Engage the lock |

Fail open during paid time, fail closed otherwise. This matters more for customer trust
than anything technical here — never punish someone because your network dropped.

⚠️ A null end time means **no deadline** (an open-ended walk-in), never "expired". Reading
null as expired would lock out a customer paying by the minute the moment the wifi
hiccuped.

The cache is written to disk rather than held in memory because the watchdog can restart
the agent while the server is unreachable — an in-memory cache would come back empty with
no idea whether the person at the machine had paid.
